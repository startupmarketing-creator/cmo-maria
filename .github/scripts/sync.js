...
function blockToObj(block) {
  ...
  switch (type) {
              ...
              case 'table_row': {
                ...
                // child_database = database created inline on the page
                // linked_database = reference/link to an existing database
              case 'child_database':
              case 'linked_database': {
                                const title = content.title || '';
                                // For linked_database, the actual database id may differ from block.id
                                const dbId = (type === 'linked_database' && content.linked_database_id)
                                  ? content.linked_database_id
                                                    : block.id;
                                console.log('  [database block] type=' + type + ' title="' + title + '" id=' + dbId);
                                return {
                                                    type: '_database',
                                                    title,
                                                    _id: dbId,
...
// Convert a database block (child_database or linked_database) to a table
async function databaseToTable(obj) {
              try {
                ...
                for (const row of rows) {
                                  const cells = colNames.map(col => propToHtml(row.properties[col] || null));
                                  tableRows.push(cells);
                }

                return { type: 'table', title: obj.title, has_header: true, rows: tableRows };
              } catch (e) {
                              console.log('  [database error] id=' + obj._id + ':', e.message);
                              // Return a placeholder so user knows the database is there but inaccessible
                return {
                                  type: 'callout',
                                  icon: '⚠️',
...
async function processBlocks(rawBlocks) {
              ...
              for (const block of rawBlocks) {
                ...
                // Database block (child or linked): fetch rows as table
                if (obj.type === '_database') {
                                  const tableObj = await databaseToTable(obj);
