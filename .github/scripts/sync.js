...
function blockToObj(block) {
  ...
  switch (type) {
          ...
          default:
                        console.log('  [unknown block type]', type, 'has_children:', block.has_children);
                        return null;
  }
}
...
// Uses getDataSourceId + dataSources.query (same pattern as Settings/Nav)
async function childDatabaseToTable(obj) {
          try {
                      console.log('  [child_database] fetching', obj.title, obj._id);
                      const dsId = await getDataSourceId(obj._id);
                      const rows = await fetchDataSource(dsId);
                      if (!rows.length) {
                                    console.log('  [child_database] no rows');
                                    return null;
                      }
                      console.log('  [child_database] got', rows.length, 'rows');
...
async function processBlocks(rawBlocks) {
          ...
          for (const block of rawBlocks) {
            ...
            // child_database: fetch rows via dataSources.query
