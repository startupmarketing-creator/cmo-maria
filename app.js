...
// Format a date range: "August 2025 — April 2026" or "August 2025 — Present"
function formatDateRange(startStr, endStr) {
const start = formatDate(startStr);
if (!start) return '';
const end = endStr ? formatDate(endStr) : 'Present';
return start + ' \u2014 ' + end;
}
...
if (card.date) html += '<p class="gallery-card-date">' + formatDateRange(card.date, card.endDate) + '</p>';
...
'<div class="work-modal-references" id="modal-references"></div>' +
...
document.getElementById('modal-date').textContent = card.date ? formatDateRange(card.date, card.endDate) : '';

// References section
const refsEl = document.getElementById('modal-references');
if (card.references && card.references.length > 0) {
refsEl.innerHTML = '<p class="work-modal-refs-label">Can recommend:</p>' +
'<ul class="work-modal-refs-list">' +
card.references.map(function(name) { return '<li>' + name + '</li>'; }).join('') +
'</ul>';
