...
// Convert child_database rows to table block
// child_database rows are fetched as pages via blocks.children.list
// then each page's properties are read via pages.retrieve
async function childDatabaseToTable(obj) {
            try {
                          console.log('  [child_database] fetching rows for:', obj.title, obj._id);

              // Get the database schema to know column names and their order
              const db = await notion.databases.retrieve({ database_id: obj._id });
                          const props = db.properties || {};
                          const colNames = Object.keys(props);
                          console.log('  [child_database] columns:', colNames.join(', '));

              // Fetch pages (rows) from the database
              // In SDK v5 with this Notion version, use dataSources if available, else use blocks
              let rows = [];
                          try {
                                          // Try: get data_source_id and use dataSources.query
                            const dsId = await getDataSourceId(obj._id);
                                          rows = await fetchDataSource(dsId);
                                          console.log('  [child_database] fetched via dataSources.query:', rows.length, 'rows');
                          } catch (e1) {
                                          // Fallback: fetch child blocks (pages) directly
                            console.log('  [child_database] dataSources failed, trying blocks.children.list:', e1.message);
                                          try {
                                                            const childBlocks = await fetchBlocks(obj._id);
                                                            // Filter only page-type children
                                            rows = childBlocks.filter(b => b.object === 'page');
                                                            console.log('  [child_database] fetched via blocks:', rows.length, 'rows');
                                          } catch (e2) {
                                                            console.log('  [child_database] blocks also failed:', e2.message);
                                                            return null;
                                          }
                          }

              if (!rows.length) {
                              console.log('  [child_database] no rows found');
                              return null;
              }

              const tableRows = [colNames]; // header
              for (const row of rows) {
                              const cells = colNames.map(col => {
                                                const prop = row.properties ? row.properties[col] : null;
                                                return propToHtml(prop);
                              });
                              tableRows.push(cells);
              }

              return { type: 'table', title: obj.title, has_header: true, rows: tableRows };
            } catch (e) {
                          console.log('  [child_database error]', e.message);
                          return null;
            }
}
            ...
            for (const block of rawBlocks) {
              ...
              if (obj.type === '_container') {
                              if (obj._has_children) {
                                                const childRaw = await fetchBlocks(obj._id);
                                                const childOut = await processBlocks(childRaw);
                                                out.push(...childOut);
                              }
                              continue;
              }
              ...
              // child_database: fetch rows
              if (obj.type === 'child_database') {
                              const tableObj = await childDatabaseToTable(obj);
                              if (tableObj) out.push(tableObj);
                              continue;
              }
...
async function buildPage(pageId) {
            ...
            console.log('  raw block types:', rawBlocks.map(b => b.type).join(', '));
