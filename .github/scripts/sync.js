...
// Extract icon emoji from callout icon object
function extractIcon(icon) {
      if (!icon) return '';
      if (icon.type === 'emoji') return icon.emoji || '';
      // For external/file icons return a generic symbol
  if (icon.type === 'external' || icon.type === 'file') return '';
      return '';
}
...
function blockToObj(block) {
  ...
  switch (type) {
      ...
      case 'child_database': {
                // Inline database — fetch its rows as a table
      ...
      default:
                // Log unknown types so we can debug
          console.log('  [skip] block type:', type);
                return null;
  }
}
...
// Fetch child_database rows using databases.query
async function fetchChildDatabase(dbId) {
      const rows = [];
      let cursor;
      do {
...
async function childDatabaseToTable(obj) {
      try {
              const rows = await fetchChildDatabase(obj._id);
              if (!rows.length) return null;

        // Get property names (columns) from first row
        const firstProps = rows[0].properties;
              const colNames = Object.keys(firstProps);

        // Build header row + data rows
        const tableRows = [];
              tableRows.push(colNames); // header
        ...
        return {
              ...
              console.log('  [child_database error]', obj._id, e.message);
              return null;
      }
}
...
async function buildPage(pageId) {
  ...
  for (const block of rawBlocks) {
        ...
        // For regular table blocks: fetch child rows and attach them
        ...
        // For child_database blocks: fetch rows via databases.query
